...
class GiftedChat extends React.Component {
    constructor(props) {
        ...
        this.onInputSizeChanged = (size) => {
            const { maxComposerHeight = 370, minComposerHeight = 50 } = this.props;
            const mediaWindowHeight = size.mediaWindowHeight || 0;
            const extraHeight = mediaWindowHeight > 0 ? 10 : 0;
            let newComposerHeight = 0;
            if (size.height > 40 && size.height < maxComposerHeight) {
                newComposerHeight = size.height + 10 + extraHeight;
            }
            else if (size.height < 40 && size.height < maxComposerHeight) {
                newComposerHeight = minComposerHeight + extraHeight;
            }
            else {
                newComposerHeight = maxComposerHeight;
            }
            const totalNewComposerHeight = newComposerHeight + mediaWindowHeight;
            const newMessagesContainerHeight = this.getMessagesContainerHeightWithKeyboard(totalNewComposerHeight);
            this.setState({
     ...
     */
    getBasicMessagesContainerHeight(composerHeight = this.state.composerHeight + this.state.mediaWindowHeight) {
        return (this.getMaxHeight() - this.calculateInputToolbarHeight(composerHeight));
    }
    /**
     * Returns the height, based on current window size, taking the keyboard into account.
     */
    getMessagesContainerHeightWithKeyboard(composerHeight = this.state.composerHeight + this.state.mediaWindowHeight) {
